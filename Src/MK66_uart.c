...
{
	uart_putchar(Printf_PORT, (char)ch);
	return(ch);
}
 ...
 *  @note       UART所用的管脚在 App\Inc\PORT_cfg.h 里进行配置，
                printf所用的管脚和波特率在 App\Inc\y_conf.h 里进行配置
...
void uart_rx_irq_en(UARTn_e uratn)
{
	...
	if(uratn<=3)	enable_irq((IRQn_t)((uratn << 1) + UART0_RX_TX_IRQn));                  //使能IRQ中断
