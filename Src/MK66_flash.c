...
//读取Flash中一段字节的数据，此函数最大读取256*4个字节的数据
void Flash_read_buf(uint16 sectNo, uint32 *bBuf, uint8 cnt)
{
	uint8 i = 0;
	cnt = cnt / 4;
	for (; i < cnt; i++){
		bBuf[i] = flash_read(sectNo, i * 4, uint32);
	}
}
...
__RAMFUNC uint8 flash_write_buf(uint16 sector_num, uint16 offset, uint16 cnt, uint32 buf[])
{
	uint32  size;
	u16 step = FLASH_ALIGN_ADDR * 2;
	uint32  addr;
	uint32  data;
	uint8 i = 0;
