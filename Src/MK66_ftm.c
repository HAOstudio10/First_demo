...
#include  "MK66_FTM.h"
   ...
   * 定义数组
   */
FTM_MemMapPtr FTMN[FTM_MAX] = { FTM0_BASE_PTR, FTM1_BASE_PTR, FTM2_BASE_PTR, FTM3_BASE_PTR
...
void ftm_pwm_init(FTMn_e ftmn, FTM_CHn_e ch, uint32 freq, uint32 duty)
{
	...
	ASSERT((ftmn == FTM0) || ((ftmn == FTM1 || ftmn == FTM2) && (ch <= FTM_CH1))|| (ftmn == FTM3)
	);  //检查传递进来的通道是否正确
